...
@bot.on(admin_cmd(pattern="kidnapall ?(.*)"))
@bot.on(sudo_cmd(pattern="kidnapall ?(.*)", allow_sudo=True))
async def get_users(event):
    sender = await event.get_sender()
    me = await event.client.get_me()
    if not sender.id == me.id:
        shinchan = await edit_or_reply(event, "`processing...`")
    else:
        shinchan = await edit_or_reply(event, "`processing...`")
    kraken = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await shinchan.edit("`Sorry, Cant kidnape users here`")
    s = 0
    f = 0
    error = "None"

    await shinchan.edit("**TerminalStatus**\n\n`kidnapeing Users.......`")
...
CmdHelp("invite").add_command(
  ...
  "kidnapeall", "<group username>", "Scraps user from the targeted group to your group. Basically Kidnapps user from one chat to another"
).add()
