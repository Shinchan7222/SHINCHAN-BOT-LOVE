...
async def _(event):
    ...
    shinchanevent = await edit_or_reply(event, "`Executing.....`")
    process = await asyncio.create_subprocess_sshinchan(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    shinchanuser = await event.client.get_me()
    if shinchanuser.username:
        curruser = shinchanuser.username
    else:
        curruser = "shinchanbot"
    uid = os.geteuid()
    if uid == 0:
...
async def _(event):
    ...
    shinchanevent = await edit_or_reply(event, "`Running ...`")
...
CmdHelp("evaluators").add_command(
  ...
  'exec', '<command>', 'Execute a Terminal command on ShinchanBot server and shows details'
).add_command(
