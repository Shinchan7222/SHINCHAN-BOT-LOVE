...
from shinchanbot.utils import admin_cmd, sudo_cmd, progress
from userbot import CMD_HELP
from userbot.cmdhelp import CmdHelp

FF_MPEG_DOWN_LOAD_MEDIA_PATH = "./downloads/shinchanbot.media.ffmpeg"
...
async def ff_mpeg_trim_cmd(event):
    ...
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        ...
        if reply_message:
            ...
            shinchanevent = await edit_or_reply(event, "`Saving the file...`")
            try:
                c_time = time.time()
                downloaded_file_name = await event.client.download_media(
                    ...
                    progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                        progress(d, t, shinchanevent, c_time, "trying to download")
                    ),
                )
            except Exception as e:
                await shinchanevent.edit(str(e))
            else:
                end = datetime.now()
...
async def ff_mpeg_trim_cmd(event):
    ...
    shinchanevent = await edit_or_reply(event, "`Triming the media......`")
    ...
    if len(cmt) == 3:
        ...
        if o is None:
            return await edit_delete(
                shinchanevent, f"**Error : **`Can't complete the process`"
            )
        try:
            c_time = time.time()
            await event.client.send_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, shinchanevent, c_time, "trying to upload")
                ),
            )
            os.remove(o)
        except Exception as e:
            return await edit_delete(shinchanevent, f"**Error : **`{e}`")
    elif len(cmt) == 2:
        ...
        await edit_delete(shinchanevent, "RTFM")
        return
    end = datetime.now()
    ms = (end - start).seconds
    await edit_delete(shinchanevent, f"`Completed Process in {ms} seconds`", 3)
