...
class ShinchanBroadcast(BASE):
    __tablename__ = "Shinchanbroadcast"
    ...
    def __repr__(self):
        return "<Shinchan Broadcast channels '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, ShinchanBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


ShinchanBroadcast.__table__.create(checkfirst=True)

ShinchanBROADCAST_INSERTION_LOCK = threading.RLock()
...
def add_to_broadcastlist(keywoard, group_id):
    with ShinchanBROADCAST_INSERTION_LOCK:
        broadcast_group = ShinchanBroadcast(keywoard, str(group_id))
...
def rm_from_broadcastlist(keywoard, group_id):
    with ShinchanBROADCAST_INSERTION_LOCK:
        broadcast_group = SESSION.query(ShinchanBroadcast).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_broadcastlist(keywoard):
    with ShinchanBROADCAST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(ShinchanBroadcast.keywoard)
            .filter(ShinchanBroadcast.keywoard == keywoard)
            .delete()
        )
...
def get_broadcastlist_chats():
    try:
        chats = SESSION.query(ShinchanBroadcast.keywoard).distinct().all()
...
def num_broadcastlist():
    try:
        return SESSION.query(ShinchanBroadcast).count()
    finally:
        SESSION.close()
...
def num_broadcastlist_chats():
    try:
        return SESSION.query(func.count(distinct(ShinchanBroadcast.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(ShinchanBroadcast).all()
        for x in all_groups:
