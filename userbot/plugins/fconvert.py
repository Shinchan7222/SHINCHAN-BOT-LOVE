...
async def _(shinchan):
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await shinchan.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await shinchan.client.send_file(
                event.chat_id,
...
async def _(event):
    if event.fwd_from:
        return
    shinchanreply = await event.get_reply_message()
    if not shinchanreply or not shinchanreply.media or not shinchanreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if shinchanreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    reply_to_id = event.message
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    chat = "@tgstogifbot"
    shinchanevent = await edit_or_reply(event, "`Converting to gif ...`")
    async with event.client.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await event.client.send_file(chat, shinchanreply.media)
            response = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await shinchanevent.edit("`This file is not supported`")
            shinchanresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            shinchanfile = Path(await event.client.download_media(shinchanresponse, "./temp/"))
            shinchangif = Path(await unzip(shinchanfile))
            ...
            for files in (shinchangif, shinchanfile):
                if files and os.path.exists(files):
                    os.remove(files)
        except YouBlockedUserError:
            await shinchanevent.edit("Unblock @tgstogifbot")
            return
