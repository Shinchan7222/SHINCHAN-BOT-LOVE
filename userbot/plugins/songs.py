...
from shinchanbot.utils import admin_cmd, sudo_cmd, edit_or_reply, progress
...
async def nope(h1m4n5hu0p):
    shinchan = h1m4n5hu0p.pattern_match.group(1)
    if not shinchan:
    ...
    troll = await bot.inline_query("iLyricsBot", f"{(deEmojify(shinchan))}")
    ...
    async with borg.conversation("@DeezLoadBot") as conv:
        try:
            ...
            await borg.send_file(
                event.chat_id,
                songh,
                caption="🔆**Here's the requested song!**🔆\n`Check out` [ShinchanBot](https://t.me/Shinchan_userbot_chat)",
            )
            await event.delete()
        except YouBlockedUserError:
