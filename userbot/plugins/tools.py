...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    shinchanevent = await edit_or_reply(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await shinchanevent.edit("`Please unblock `@DrWebBot `and try again`")
            return
...
async def _(event):
    if event.fwd_from:
        return
    shinchanevent = await edit_or_reply(event, "...")
    start = datetime.now()
    ...
    await shinchanevent.edit("Created BarCode in {} seconds🤓".format(ms))
...
async def _(event):
    if event.fwd_from:
        return
    shinchanevent = await edit_or_reply(event, "`processiong...........`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await shinchanevent.edit(output_str, link_preview=True)
    else:
        await shinchanevent.edit("xkcd n.{} not found!".format(xkcd_id))
...
CmdHelp("tools").add_command(
  ...
  "currency", "<amount> <from> <to>", "Currency converter for ShinchanBot", ".currency 10 usd inr"
).add_command(
