...
from shinchanbot import CmdHelp, bot as Shinchanbot, ALIVE_NAME
from shinchanbot.utils import admin_cmd, sudo_cmd, edit_or_reply as eor
...
@shinchanbot.on(admin_cmd(pattern="hvd$"))
@shinchanbot.on(sudo_cmd(pattern="hvd$", allow_sudo=True))
...
@shinchanbot.on(admin_cmd(pattern="vday$"))
@shinchanbot.on(sudo_cmd(pattern="vday$", allow_sudo=True))
async def gif(event):
    if event.fwd_from:
        return
