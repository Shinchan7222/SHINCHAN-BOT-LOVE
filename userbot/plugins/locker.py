...
async def locks(event):
    ...
    try:
        ...
        await edit_or_reply(event, f"[{DEFAULTUSER}](tg://user?id={Shinchan7222}) Locked `{what}` \n__Cause its Rest Time Nimba!!__")
...
async def rem_locks(event):
    ...
    try:
        ...
        await edit_or_reply(event, f"[{DEFAULTUSER}](tg://user?id={Shinchan7222}) Unlocked `{what}` \n__Now Start Chit Chat !!__")
    except BaseException as e:
        await edit_or_reply(event, f"`Do I have proper rights for that ??`\n**Error:** {str(e)}")
        return
