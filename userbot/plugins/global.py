                                 ...
                                 ChatAdminRights,
                                   ChatBannedRights,
                                     MessageEntityMentionName,
                                       MessageMediaPhoto)
from shinchanbot.utils import register, errors_handler
from shinchanbot.utils import admin_cmd
...
async def gben(userbot):
    ...
    try:
        ...
        pass
    if me == user:
       k = await shinchan.edit("U want to promote urself 😑😑 waao..")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    except:
        return await shinchan.edit(f"**Something W3NT Wrong 🤔**")
    if user:
        ...
        for x in telchanel:
          try:
             ...
             i += 1
             await shinchan.edit(f"**Promoted in Chats **: `{i}`")
          except:
             pass
    else:
        await shinchan.edit(f"**Reply to a user you dumbo !!**")
    return await shinchan.edit(
...
async def gben(userbot):
    ...
    try:
        ...
        pass
    if me == user:
       k = await shinchan.edit("U want to demote urself 😑😑 waao..")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    ...
    if user:
        ...
        for x in telchanel:
          try:
             await userbot.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
             await shinchan.edit(f"**Demoted in Chats **: `{i}`")
          except:
             pass
    else:
        await shinchan.edit(f"**Reply to a user you dumbo !!**")
