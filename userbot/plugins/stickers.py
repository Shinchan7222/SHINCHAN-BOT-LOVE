...
shinchanbot = Config.CUSTOM_STICKER_PACK_NAME
...
async def kang(args):
    ...
    if photo:
        ...
        packname = f"HB_{user.username}_ShinchanBot_{pack}"
        packnick = (
            f"{shinchanbot} Vol.{pack}"
            if shinchanbot
            else f"@{user.username}'s ShinchanBot Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
