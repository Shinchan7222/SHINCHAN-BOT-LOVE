...
from shinchanbot.Config import Config
...
async def bigspam(shinchan):
    if not shinchan.text[0].isalpha() and shinchan.text[0] not in ("/", "#", "@", "!"):
        shinchan_msg = shinchan.text
        shinchanbot_count = int(shinchan_msg[9:13])
        shinchan_spam = str(shinchan.text[13:])
        for i in range(1, shinchanbot_count):
            await shinchan.respond(shinchan_spam)
        await shinchan.delete()
        if LOGGER:
            await shinchan.client.send_message(
...
CmdHelp("spam").add_command(
  ...
  "bigspam", "<count> <text>", "Sends the text 'X' number of times. This what Shinchanbot iz known for. The Best BigSpam Ever", ".bigspam 5000 Hello"
).add()
