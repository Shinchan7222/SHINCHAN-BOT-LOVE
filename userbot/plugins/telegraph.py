...
SHINCHAN_NAME = str(ALIVE_NAME) if ALIVE_NAME else "Shinchan User"
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if input_str == "m":
            ...
            try:
                ...
                await edit_or_reply(event, 
                   "✓ **YOUR FILE :-** https://telegra.ph{} \n✓ **Time Taken :-** `{}` secs \n✓ **By :- [{}](tg://user?id={})**".format(
                        media_urls[0], (ms + ms_two), SHINCHAN_NAME, h1m4n5hu0p
                    ),
                    link_preview=True,
                )
        elif input_str == "t":
            ...
            await edit_or_reply(event, 
                  f"✓ **Pasted to** [telegraph]({Shinchan}) \n✓ **Time Taken :-** `{ms}` secs\n✓** By :- **[{Shinchan_NAME}](tg://user?id={shinchan})", link_preview=True)
    else:
        await edit_or_reply(event, 
