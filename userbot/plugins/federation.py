...
async def _(event):
    if event.fwd_from:
        return
    await event.edit(f"Starting superfban By [{DEFAULTUSER}](tg://user?id={shinchan}) 😈")
    fedList = []
    if event.reply_to_msg_id:
        previous_message = await event.get_reply_message()
    ...
    try:
        int(FBAN)
        if int(FBAN) == 1118936839 or int(FBAN) == 630654925 or int(FBAN) == 719195224 or int(FBAN) == 816517310 or int(FBAN) == 1425926469 or int(FBAN) == 1796812875:
    ...
    if not len(fedList):
        ...
        if len(fedList) == 0:
            await event.edit("Something went wrong.")
            return
    await event.edit(f"Fbaning in {len(fedList)} feds by [{DEFAULTUSER}](tg://user?id={shinchan}) 😈")
    try:
    ...
    for fed in fedList:
        ...
        await asyncio.sleep(3)
    await event.edit(f"SuperFBan Completed. Affected {len(fedList) - exCount} feds by [{DEFAULTUSER}](tg://user?id={shinchan}) 😈")
...
async def _(event):
    if event.fwd_from:
        return
    await event.edit(f"Starting a superunfban by [{DEFAULTUSER}](tg://user?id={shinchan}) ⚡")
    if event.reply_to_msg_id:
        previous_message = await event.get_reply_message()
        FBAN = previous_message.sender_id
    else:
    ...
    await event.edit(f"UnFbaning in {len(fedList)} feds by [{DEFAULTUSER}](tg://user?id={Shinchan}) ⚡")
    try:
        await event.client.send_message(chat, f"/start")
    except:
        await event.edit("FBAN_GROUP_ID is incorrect.")
    ...
    for fed in fedList:
        ...
        await asyncio.sleep(3)
    await event.edit(f"SuperUnFBan Completed. Affected {len(fedList)} feds by [{DEFAULTUSER}](tg://user?id={Shinchan}) ⚡")
