...
# Now will be used in shinchanlBot too....
import asyncio
import datetime
...
from shinchanbot.utils import admin_cmd, edit_or_reply
...
async def set_not_afk(event):
    ...
    if ".afk" not in current_message and "yes" in USER_AFK:  # pylint:disable=E0602
        shinchanbot = await borg.send_message(
            event.chat_id,
            "🔥__Back alive!__\n**No Longer afk.**\n⏱️ `Was afk for:``"
            + total_afk_time
            + "`", file=shinchanpic
        ...
        try:
            ...
            await borg.send_message(  # pylint:disable=E0602
                ...
                + "Ask in @Shinchan_userbot_chat to get help setting this value\n\n `{}`".format(str(e)),
                reply_to=event.message.id,
                silent=True,
            )
        await asyncio.sleep(5)
        await shinchanbot.delete()
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:
        ...
        message_to_reply = (
            f"Hey!! My Legend master [{DEFAULTUSER}](tg://user?id={shinchan}) is currently offline... Since when?\n**For** `{total_afk_time}`\n"
            + f"\n\n👇__The Reason Is__👇 :-\n`{reason}`"
  if reason
            else f"**Heyy!**\n__I am currently unavailable.__\n__Since when, you ask? From__ `{total_afk_time}`\nI'll be back when I feel to come🚶"
        )
        msg = await event.reply(message_to_reply, file=shinchanpic)
...
async def _(event):
    ...
    shinchanpic = await event.client.download_media(h1m4n5hu0p)
    if not USER_AFK:  # pylint:disable=E0602
        ...
        USER_AFK = f"yes: {reason} {shinchanpic}"  # pylint:disable=E0602
        if reason:
            await borg.send_message(
                event.chat_id, f"__**I'm going afk🚶**__ \n⚜️ Because `{reason}`", file=shinchanpic
            )
        else:
            await borg.send_message(event.chat_id, f"**I am Going afk!**🚶", file=shinchanpic)
        await asyncio.sleep(0.001)
        await event.delete()
        try:
            await borg.send_message(  # pylint:disable=E0602
                Config.PRIVATE_GROUP_BOT_API_ID,  # pylint:disable=E0602
                f"#AFKTRUE \nSet AFK mode to True, and Reason is {reason}",file=shinchanpic
            )
        except Exception as e:  # pylint:disable=C0103,W0703
