...
async def promote(promt):
    ...
    shinchanevent = await edit_or_reply(promt, "Promoting...")
    ...
    try:
        ...
        await shinchanevent.edit("Promoted Successfully!")
    except BadRequestError:
        await shinchanevent.edit(NO_PERM)
        return
    if BOTLOG:
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    shinchanevent = await edit_or_reply(dmod, "Demoting...")
    rank = "??????"
    ...
    try:
        ...
        await shinchanevent.edit(NO_PERM)
        return
    await shinchanevent.edit("Demoted Successfully!")
    if BOTLOG:
...
async def ban(bon):
    ...
    if not user:
        return
    shinchanevent = await edit_or_reply(bon, "Banning this retard")
    try:
    ...
    try:
        ...
        await shinchanevent.edit("I ain't got msg deleting right. But still Banned!")
        return
    if reason:
        await shinchanevent.edit(f"{str(user.id)} is banned !!\nReason: {reason}")
    else:
        await shinchanevent.edit(f"{str(user.id)} is banned!")
    if BOTLOG:
...
async def nothanos(unbon):
    ...
    if not admin and not creator:
        ...
        return
    shinchanevent = await edit_or_reply(unbon, "Unbanning...")
    ...
    try:
        ...
        await shinchanevent.edit("Unbanned Successfully!")
        if BOTLOG:
        ...
        await shinchanevent.edit("Sorry I Can't Unban This Retard!")
...
async def kick(usr):
    ...
    if not user:
        ...
        return
    shinchanevent = await edit_or_reply(usr, "Kicking...")
    try:
        await usr.client.kick_participant(usr.chat_id, user.id)
        await sleep(0.5)
    except Exception as e:
        await shinchanevent.edit(NO_PERM + f"\n{str(e)}")
        return
    if reason:
        ...
        await shinchanevent.edit(f"Kicked [{user.first_name}](tg://user?id={user.id})!")
    if BOTLOG:
