...
async def admin_groups(shinchan):
    shinchangroups = []
    async for dialog in shinchan.client.iter_dialogs():
        ...
        if (
            ...
            shinchangroups.append(entity.id)
    return shinchangroups
...
async def make_gif(event, file):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await event.client.send_read_acknowledge(conv.chat_id)
            shinchanfile = await event.client.download_media(shinchanresponse, "./temp")
            return await unzip(shinchanfile)
...
async def waifutxt(text, chat_id, reply_to_id, bot, borg):
    ...
    shinchan = await sticcers[0].click("me", hide_via=True)
    if shinchan:
        await bot.send_file(int(chat_id), Shinchan, reply_to=reply_to_id)
