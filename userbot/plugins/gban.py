...
HIMANSHU = str(ALIVE_NAME) if ALIVE_NAME else "Shinchan User"
papa = borg.uid
...
async def gban(userbot):
    ...
    if not sender.id == hum.id:
        ...
        shinchanbot = await edit_or_reply(ids, "`Ok! Gbaning this piece of shit....`")
    hum = await userbot.client.get_me()
    await shinchanbot.edit(f"`🔥Global Ban Iz Cumin💦.... Wait and watch nigga🚶`")
    ...
    if user:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await shinchanbot.edit(f"Gbaning This retard🚶\n\n**Please Wait Few Minutes**😏")
            except:
                b += 1
    else:
        await shinchanbot.edit(f"`Either reply to a user or gib me user id/name`")
    try:        
...
async def gunban(userbot):
    ...
    hum = await ids.client.get_me()
    if not sender.id == hum.id:
        shinchanbot = await edit_or_reply(ids, "`Trying to ungban this kid...`")
    else:
    ...
    if user:
        if user.id == 816517310 or user.id == 1212368262:
            return await shinchanbot.edit("**You need to grow some balls to gban / ungban my creator and his freands**")
        try:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await shinchanbot.edit(f"Ok! Now Ungbaning this kiddo.\n\n**Please Wait Few Minutes**😏")
            except:
                b += 1
    else:
    ...
    try:
        if ungmute(user.id) is False:
            return await shinchanbot.edit("**Error! I think User already ungbanned.**")
    except:
        pass
    return await shinchanbot.edit(
