...
async def download(event):
    ...
    if Var.GIT_REPO_NAME is None:
        await edit_or_reply(event, "`Please ADD Proper Github Repo Name of shinchanBot`")
        return
    shinchanbot = await edit_or_reply(event, "Processing ...")
    ...
    try:
        ...
        await shinchanbot.edit("Committing to Github....")
        await git_commit(downloaded_file_name, shinchanbot)


async def git_commit(file_name, shinchanbot):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await shinchanbot.edit("`File Already Exists`")
            create_file = False
    ...
    if create_file == True:
        ...
        try:
            ...
            await shinchanbot.edit("Cannot Upload Plugin")
    else:
        return await shinchanbot.edit("`Committed Suicide`")
