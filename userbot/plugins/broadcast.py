...
async def shinchanbroadcast_send(event):
    if event.fwd_from:
        return
    shinchaninput_str = event.pattern_match.group(1)
    if not shinchaninput_str:
    ...
    keyword = shinchaninput_str.lower()
    ...
    shinchanevent = await edit_or_reply(
        event,
    ...
    await shinchanevent.edit(resultext)
    if BOTLOG:
...
async def shinchanbroadcast_add(event):
    if event.fwd_from:
        return
    shinchaninput_str = event.pattern_match.group(1)
    if not shinchaninput_str:
...
async def shinchanbroadcast_remove(event):
    if event.fwd_from:
        return
    shinchaninput_str = event.pattern_match.group(1)
    if not shinchaninput_str:
...
async def shinchanbroadcast_list(event):
    if event.fwd_from:
        return
    shinchaninput_str = event.pattern_match.group(1)
    if not shinchaninput_str:
    ...
    await edit_or_reply(shinchanevent, finaloutput)
