...
shinchan = f"[{DEFAULTUSER}](tg://user?id={shinchan7222})"
...
async def send(event):
    ...
    omk = f"**⍟ Plugin name ≈** `{input_str}`\n**⍟ Uploaded by ≈** {shinchan}\n\n⚡ **[LEGENDARY AF SHINCHANBOT](https://t.me/Shinchan_USERBOT)** ⚡"
...
@shinchanbot.on(admin_cmd(pattern="install$", outgoing=True))
@shinchanbot.on(sudo_cmd(pattern="install$", allow_sudo=True))
                ...
                if shortname in CMD_LIST:
                    ...
                    for i in CMD_LIST[shortname]:
                        ...
                        if b == 1:
                            ...
                            b = 1
                        await event.edit(a)
                    return await event.edit(f"✅ **Installed module** :- `{shortname}` \n✨ BY :- {shinchan}\n\n{string}\n\n        ⚡ **[LEGENDARY AF SHINCHANBOT](https://t.me/Shinchan_USERBOT)** ⚡", link_preview=False)
...
@shinchanbot.on(admin_cmd(pattern=r"uninstall (?P<shortname>\w+)", outgoing=True))
@shinchanbot.on(sudo_cmd(pattern=r"uninstall (?P<shortname>\w+)", allow_sudo=True))
async def uninstall(shinchan):
...
@shinchanbot.on(admin_cmd(pattern=r"unload (?P<shortname>\w+)$"))
@shinchanbot.on(sudo_cmd(pattern=r"upload (?P<shortname>\w+)$", allow_sudo=True))
...
@shinchanbot.on(admin_cmd(pattern=r"load (?P<shortname>\w+)$"))
@shinchanbot.on(sudo_cmd(pattern=r"load (?P<shortname>\w+)$", allow_sudo=True))
...
CmdHelp("core").add_command(
  ...
  "cmds", None, "Gives out the list of modules in Shinchanbot."
).add()
